...
const BoardRenderer = {
  build(svgEl, grid, labels, opts = {}) {
    ...
    BoardRenderer._defs(svgEl, ns, R, hatchId);
    ...
    for (const c of grid.cells.values()) {
      ...
      BoardRenderer._cell(svgEl, ns, c, x+ox, y+oy, R, lmap[HexGrid.key(c.q,c.r)]??null, opts.hover!==false, hatchId);
    }
  },

  _defs(svgEl, ns, R, hatchId) {
    const defs=document.createElementNS(ns,'defs'), sz=Math.max(2.5,R*0.2), lw=Math.max(0.6,sz*0.3);
    ...
    bg.setAttribute('width',sz.toFixed(1)); bg.setAttribute('height',sz.toFixed(1)); bg.setAttribute('fill','#111');
    pat.appendChild(bg);
    const ln=document.createElementNS(ns,'line');
    ln.setAttribute('x1','0'); ln.setAttribute('y1','0'); ln.setAttribute('x2','0'); ln.setAttribute('y2',sz.toFixed(1));
    ln.setAttribute('stroke','#555'); ln.setAttribute('stroke-width',lw.toFixed(1));
  ...
  _cell(svgEl, ns, cell, cx, cy, R, label, hover, hatchId) {
    ...
    face.classList.add('cell-face'); BoardRenderer._fill(face, cell.state, hatchId); g.appendChild(face);
    if (label) {
      ...
      t.setAttribute('fill',cell.state===1?'#2a2a2a':'#909090'); t.setAttribute('pointer-events','none');
      t.textContent=label; g.appendChild(t);
    } else if (R>9) {
      const d=document.createElementNS(ns,'circle');
      d.setAttribute('cx',cx.toFixed(2)); d.setAttribute('cy',cy.toFixed(2)); d.setAttribute('r',(R*0.07).toFixed(2));
      d.setAttribute('fill','#2a2a2a'); d.setAttribute('pointer-events','none'); d.classList.add('cell-dot'); g.appendChild(d);
    }
    if (hover) {
      g.addEventListener('mouseenter',()=>{ if(!cell.state) face.setAttribute('fill','#1e1e1e'); });
      g.addEventListener('mouseleave',()=>BoardRenderer._fill(face, cell.state, hatchId));
    }
    svgEl.appendChild(g);
  },

  _fill(face, s, hatchId) {
    const h = hatchId || face.closest?.('svg')?._hatchId || 'hatch-o';
    if      (!s)    { face.setAttribute('fill','#1c1c1c'); face.setAttribute('stroke','#2e2e2e'); face.setAttribute('stroke-width','0.9'); }
    else if (s===1) { face.setAttribute('fill','#c8c8c8'); face.setAttribute('stroke','none'); }
    else            { face.setAttribute('fill',`url(#${h})`); face.setAttribute('stroke','#888'); face.setAttribute('stroke-width','1'); }
  },

  updateCell(svgEl, q, r, state) {
    const g=svgEl.querySelector(`[data-q="${q}"][data-r="${r}"]`);
    const f=g?.querySelector('.cell-face'); if (f) BoardRenderer._fill(f, state, svgEl._hatchId);
    const t=g?.querySelector('text'); if (t) t.setAttribute('fill',state===1?'#2a2a2a':'#909090');
  },
};
